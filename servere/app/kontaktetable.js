...
function cancelarNuevoRegistro(btn) {
  ...
  /*function crearYMostrarTabla(datos) {
    ...
    datos.forEach(function(objeto) {
      var tr = document.createElement('tr');
      var accionesTd = document.createElement('td');
      accionesTd.innerHTML = '<button class="edit-btn btn btn-outline-primary " onclick="editarRegistro(this)"><i class="fas fa-edit"></i></button>' +
                             '<button class="delete-btn btn btn-outline-danger btn-outline-primary" onclick="eliminarRegistro(this)"><i class="fas fa-trash-alt"></i></button>' +
                             '<button class="save-btn btn btn-outline-success btn-outline-primary" onclick="guardarRegistro(this)"><i class="fas fa-save"></i></button>' +
                             '<button class="cancel-btn btn btn-outline-secondary btn-outline-primary" onclick="cancelarEdicion(this)"><i class="fas fa-times"></i></button>';
      tr.appendChild(accionesTd);
      columnas.forEach(function(columna) {
        var td = document.createElement('td');
        td.textContent = objeto[columna];
    ...
    if (contenedor) {
      contenedor.innerHTML = '';
      contenedor.style.overflowY = 'scroll';
      contenedor.style.maxHeight = '70%'; // Ajusta la altura según sea necesario
      contenedor.style.width = '70%'; // Establece el ancho de la tabla
      tabla.style.position = 'absolute'; // Establece el ancho de la tabla
      contenedor.appendChild(tabla);
    } else {
      console.error('No se encontró el elemento contenedor para la tabla.');
    }
