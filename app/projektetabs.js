  ...
  var projektId = id;
    $.ajax({
      ...
      success: function(data) {
        let projectHtml = '<button class="btn btn-primary" data-toggle="modal" data-target="#addModal"><i class="fas fa-plus"></i> Agregar Nuevo Hito</button>';
        ...
        $('#tab-container').html(projectHtml);
      },
      error: function(err) {
        console.error(err);
        $('#tab-container').html('<div class="alert alert-danger" role="alert">Error al cargar los detalles del proyecto.</div>');
      }
    });
  }
  ...
  function addNewHito() {
    ...
    $.ajax({
      ...
      success: function(response) {
        $('#addModal').modal('hide');
        loadLeistungsphasenDetails(1); // Recargar los detalles del proyecto
      },
      error: function(err) {
        console.error(err);
